/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   Reviewed and commented on Nov. 2019 by Renan de Luca Avila  (renan.avila@usp.br)
   Public domain code.
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
sudoku(Rows) :-
        length(Rows, 9), maplist(same_length(Rows), Rows), /*Todas as linhas devem ter 9 elementos*/
        append(Rows, Vs), Vs ins 1..9, /*Cada elemento de cada linha deve ser um inteiro entre 1 e 9*/
        maplist(all_distinct, Rows), /*Todos os elementos numa dada linha devem ser diferentes*/
        transpose(Rows, Columns), maplist(all_distinct, Columns),  /*Todos os elementos numa dada coluna devem ser diferentes*/
        Rows = [As,Bs,Cs,Ds,Es,Fs,Gs,Hs,Is],  /*Nomeando as linhas de cima para baixo*/
        blocks(As, Bs, Cs), blocks(Ds, Es, Fs), blocks(Gs, Hs, Is). /*Regra de verificar quadrados para todas as 3 triplas de linhas internos*/

/*Definição do predicado da regra de quadrados 3x3*/
blocks([], [], []). /*default, ocorre caso os argumentos sejam vazios*/
blocks([N1,N2,N3|Ns1], [N4,N5,N6|Ns2], [N7,N8,N9|Ns3]) :- /*para uma dada sequência de linhas, pegar os 3 primeiros de cada uma e alocar em N1..N9*/
        all_distinct([N1,N2,N3,N4,N5,N6,N7,N8,N9]), /*Os elementos de um quadrado 3x3 devem ser diferentes*/
        blocks(Ns1, Ns2, Ns3). /*Em uma tripla de linha, propagar e pegar os 3 quadrados como colunas*/
